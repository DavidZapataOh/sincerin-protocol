...
export const ENCRYPTED_TOKEN_CONTRACT_ID = "CC72T3L7KMCVJ2FSW4XIZCQFRNXCVLQGVR7AYMMMFWRQHHEWOFV4TDNS"
 ...
 */
async function createEncryptedIndex(userAddress: string, serverAddress: string): Promise<Uint8Array> {
...
export async function convertToPrivate(
  ...
  try {
    ...
    const serverManagerOperation = encryptedTokenContract.call("get_server_manager")
    ...
    const serverManagerSim = await sorobanRpc.simulateTransaction(serverManagerTx)
    if (rpc.Api.isSimulationError(serverManagerSim)) {
      console.error("Server manager simulation error:", serverManagerSim.error)
      const errorMsg = typeof serverManagerSim.error === 'string' 
        ? serverManagerSim.error 
        : JSON.stringify(serverManagerSim.error)
      throw new Error(`Failed to get server manager address. The contract may not be initialized. Error: ${errorMsg}`)
    }

    if (!serverManagerSim.result || !serverManagerSim.result.retval) {
      console.error("Server manager simulation result:", serverManagerSim.result)
      throw new Error("No result from get_server_manager simulation. The contract may not be initialized.")
    }
    ...
    if (!serverManagerAddress || serverManagerAddress.length < 10) {
      throw new Error(`Invalid server manager address: ${serverManagerAddress}`)
    }
    
    console.log("Server manager address retrieved:", serverManagerAddress)
    
    // Create encrypted index
    const encryptedIndex = await createEncryptedIndex(userAddress, serverManagerAddress)
    ...
    const contractAddressScVal = Address.fromString(ENCRYPTED_TOKEN_CONTRACT_ID).toScVal()
    const amountScVal = nativeToScVal(amountInSmallestUnit, { type: "i128" })
    ...
    const transferTx = new TransactionBuilder(sourceAccount, {
      ...
      .addOperation(
        usdcContract.call("transfer", userAddressScVal, contractAddressScVal, amountScVal)
      )
      .setTimeout(300)
      .build()
    ...
    const { signedTxXdr: signedTransferXdr } = await kit.signTransaction(transferXdr, {
      networkPassphrase: STELLAR_NETWORK,
    })
    ...
    const signedTransferTx = TransactionBuilder.fromXDR(signedTransferXdr, STELLAR_NETWORK)
    const transferResponse = await sorobanRpc.sendTransaction(signedTransferTx)
    
    if (transferResponse.errorResult) {
      throw new Error(`Transfer failed: ${JSON.stringify(transferResponse.errorResult)}`)
    }
    ...
    const encryptedIndexScVal = nativeToScVal(encryptedIndexBuffer, { type: "bytes" })
    ...
    const preparedDepositTx = await sorobanRpc.prepareTransaction(requestDepositTx)
    
    // Sign and submit request_deposit
    const depositXdr = preparedDepositTx.toXDR()
    const { signedTxXdr: signedDepositXdr } = await kit.signTransaction(depositXdr, {
      networkPassphrase: STELLAR_NETWORK,
    })
    ...
    const signedDepositTx = TransactionBuilder.fromXDR(signedDepositXdr, STELLAR_NETWORK)
    const depositResponse = await sorobanRpc.sendTransaction(signedDepositTx)
    
    if (depositResponse.errorResult) {
      throw new Error(`Deposit request failed: ${JSON.stringify(depositResponse.errorResult)}`)
    }
...
export async function getPrivateBalance(userAddress: string): Promise<string> {
  try {
    ...
    const getIndexTx = new TransactionBuilder(sourceAccount, {
      ...
      .addOperation(
        encryptedTokenContract.call("get_user_index_by_address", userAddressScVal)
      )
      .setTimeout(30)
      .build()
    ...
    if (!encryptedIndexBytes || (Array.isArray(encryptedIndexBytes) && encryptedIndexBytes.length === 0)) {
      return "0"
    }
    ...
    if (Array.isArray(encryptedIndexBytes)) {
      ...
      const cleanHex = encryptedIndexBytes.startsWith('0x') ? encryptedIndexBytes.slice(2) : encryptedIndexBytes
      encryptedIndexArray = new Uint8Array(Buffer.from(cleanHex, 'hex'))
    } else {
      encryptedIndexArray = new Uint8Array(Buffer.from(encryptedIndexBytes as any))
    }
    ...
    const userIndexHash = CryptoJS.SHA256(CryptoJS.enc.Hex.parse(encryptedIndexHex))
    const userIndexArray = new Uint8Array(32)
    const userIndexWords = userIndexHash.words
    for (let i = 0; i < 32; i++) {
      const wordIndex = i >>> 2
      const byteIndex = i % 4
      userIndexArray[i] = (userIndexWords[wordIndex] >>> (24 - byteIndex * 8)) & 0xff
    }
    ...
    if (Array.isArray(encryptedKeyUserBytes)) {
      ...
      const cleanHex = encryptedKeyUserBytes.startsWith('0x') ? encryptedKeyUserBytes.slice(2) : encryptedKeyUserBytes
      encryptedKeyUserArray = new Uint8Array(Buffer.from(cleanHex, 'hex'))
    } else {
      encryptedKeyUserArray = new Uint8Array(Buffer.from(encryptedKeyUserBytes as any))
    }
    ...
    for (let i = 0; i < 32 && i < decryptedKey.sigBytes; i++) {
      const wordIndex = i >>> 2
      const byteIndex = i % 4
      symmetricKeyArray[i] = (decryptedKeyWords[wordIndex] >>> (24 - byteIndex * 8)) & 0xff
    }
    ...
    if (Array.isArray(encryptedAmountBytes)) {
      ...
      const cleanHex = encryptedAmountBytes.startsWith('0x') ? encryptedAmountBytes.slice(2) : encryptedAmountBytes
      encryptedAmountArray = new Uint8Array(Buffer.from(cleanHex, 'hex'))
    } else {
      encryptedAmountArray = new Uint8Array(Buffer.from(encryptedAmountBytes as any))
    }
